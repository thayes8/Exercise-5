...
```
It looks like the initial outbreak in 2020 isn't as signifigant as the second spike at the end of 2020 which appears by the much higher numbers evident by the brighter blue colors. A flaw in this graph is that it only takes into account cumulative cases which means that we don't get an active case count at different times to show more accurately how the covid numbers are changing month by month. Also it looks as though it only changes signifigantly once in this graph on 11-15-20 which doesn't seem right.
